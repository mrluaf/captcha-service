...
var deathbycaptcha = {
  setApiKey: function (credentials) {
    const crsplit = credentials.split('|') || ['', ''];
    deathbycaptcha.credentials = {
      username: (crsplit.length == 2) ? crsplit[0] : '',
      password: (crsplit.length == 2) ? crsplit[1] : ''
    };
    return true;
  },
  ...
  decodeFile: function (filePath, loopDelay, cb) {
    ...
    var file_reader = fs.createReadStream(filePath, {
      encoding: 'binary'
    });
    var file_contents = '';
  ...
  upload: function (binaryContents, contentType, loopDelay, cb) {
    ...
    var post_data = [];
    post_data.push(new Buffer.from(encodeFieldPart(boundary, 'username', deathbycaptcha.credentials.username), 'ascii'));
    post_data.push(new Buffer.from(encodeFieldPart(boundary, 'password', deathbycaptcha.credentials.password), 'ascii'));
    post_data.push(new Buffer.from(encodeFilePart(boundary, contentType, 'captchafile', 'mycaptcha.' + mime.extension(contentType)), 'binary'));
    post_data.push(new Buffer.from(binaryContents, 'binary'));
  ...
  balanceZero: function (cb) {
    deathbycaptcha.credit((err, result) => {
      if (err) {
        cb(err, null);
      } else {
        if (result.balance == '0' || result.balance == '0.0') {
          cb(null, true);
        } else {
          cb(null, false);
        }
